...
def generate_seed_data(
    ...
    if use_subfolders:
        ...
        print(f"Generating {count} artwork images across {len(subfolders)} subfolders...")
    ...
    for i in range(count):
        ...
        # Determine output path (with or without subfolder)
        if subfolders:
            subfolder = random.choice(subfolders)
            file_output_dir = output_dir / subfolder
        else:
            file_output_dir = output_dir
        
        # Create the artwork image
        create_artwork_image(name, file_output_dir / artwork_filename)
...
Examples:
  ...
  %(prog)s -n 200 -o /tmp/kartend-seed
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Number of subfolders to create (default: random 5-15)'
    )
    ...
    generate_seed_data(
        ...
        subfolder_count=args.subfolder_count,
    )
