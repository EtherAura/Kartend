...
private slots:
  ...
  // quoteForCmdExe tests (Kartend-9u29e)
...
// PathUtils::quoteForCmdExe — applying it must recover the layer-1
...
void TestPathUtils::testQuoteForCmdExe_exact_data() {
...
void TestPathUtils::testQuoteForCmdExe_exact() {
  QFETCH(QString, arg);
  QFETCH(QString, expected);
  QCOMPARE(PathUtils::quoteForCmdExe(arg), expected);
}

void TestPathUtils::testQuoteForCmdExe_roundTrip_data() {
  QTest::addColumn<QString>("arg");
...
void TestPathUtils::testQuoteForCmdExe_roundTrip() {
  QFETCH(QString, arg);
  ...
  const QString encoded = PathUtils::quoteForCmdExe(arg);
