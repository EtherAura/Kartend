...
[[nodiscard]] inline int computeItemY(int index, int gridWidth, int itemHeight,
...
[[nodiscard]] inline int computeCenterTarget(int itemCoord, int itemSize,
                                             int viewportSize, int maxScroll) {
...
inline void calculateGridMetrics(int totalItems, int itemsPerRow, int itemWidth,
                                 int itemHeight, int horizontalSpacing,
                                 int verticalSpacing, int margins,
                                 int &totalWidth, int &totalHeight,
                                 int &actualGridWidth, int &logicalHeight,
                                 double &scrollScale, bool &isClipped) {
  int totalRows =
      (itemsPerRow > 0) ? ((totalItems + itemsPerRow - 1) / itemsPerRow) : 1;

  if (itemsPerRow > 0) {
    int horizontalSpacingContribution =
        (itemsPerRow > 1 ? (itemsPerRow - 1) * horizontalSpacing : 0);
    int calculatedWidth =
        margins * 2 + itemsPerRow * itemWidth + horizontalSpacingContribution;
  ...
  int verticalSpacingContribution =
      (totalRows > 1 ? (totalRows - 1) * verticalSpacing : 0);
  ...
  logicalHeight =
      static_cast<int>(qMin(rawHeight, static_cast<qint64>(INT_MAX)));
  ...
  if (rawHeight > MAX_HEIGHT) {
    ...
    scrollScale =
        static_cast<double>(rawHeight) / static_cast<double>(MAX_HEIGHT);
    isClipped = true;
  } else {
...
inline void calculateGridMetrics(int totalItems, int itemsPerRow, int itemWidth,
  ...
  calculateGridMetrics(totalItems, itemsPerRow, itemWidth, itemHeight,
                       horizontalSpacing, verticalSpacing, margins, totalWidth,
                       totalHeight, actualGridWidth, logicalHeight, scrollScale,
                       isClipped);
}
