...
static auto ensureUniqueIndexItemsUuidPath(QSqlDatabase &db,
  ...
  if (!errText.contains("unique", Qt::CaseInsensitive) &&
    ...
    auto err = ErrorContext::warning(ErrorCode::DatabaseQueryFailed,
                                     "Failed to ensure unique index uniq_items_uuid_path",
                                     origin)
                   .withDetails(errText);
    ErrorUtils::logError(err);
    return;
  ...
  QSqlQuery dedupe(db);
  dedupe.exec(
      "DELETE FROM items WHERE rowid NOT IN (SELECT MIN(rowid) FROM items GROUP BY collection_uuid, path)");
  QSqlQuery retry(db);
  if (!retry.exec(
          "CREATE UNIQUE INDEX IF NOT EXISTS uniq_items_uuid_path ON items(collection_uuid, path)")) {
    auto err = ErrorContext::warning(ErrorCode::DatabaseQueryFailed,
                                     "Failed to create unique index after de-duplication",
                                     origin)
...
static auto ensureIndex(QSqlDatabase &db, const QString &sql,
  ...
  auto err = ErrorContext::warning(ErrorCode::DatabaseQueryFailed,
                                   "Failed to ensure database index",
                                   origin)
...
void applySchemaMigrations(QSqlDatabase &db, const QString &origin) {
  ...
  if (mutableVersion < 2) {
    // v2: Add query performance indexes used by the virtual scroll + filtering paths.
    ensureIndex(db,
    ...
    ensureIndex(db,
                "CREATE INDEX IF NOT EXISTS idx_items_uuid_name ON items(collection_uuid, name COLLATE NOCASE)",
                origin, "idx_items_uuid_name");
    ensureIndex(db,
                "CREATE INDEX IF NOT EXISTS idx_items_covering ON items(collection_uuid, name COLLATE NOCASE, path)",
                origin, "idx_items_covering");
    ensureIndex(db,
                "CREATE INDEX IF NOT EXISTS idx_items_uuid_last_modified ON items(collection_uuid, last_modified)",
  ...
  if (mutableVersion < 3) {
    ...
    // This is best-effort: if SQLite is built without FTS5, keep working with LIKE.
    {
      QSqlQuery q(db);
      ...
      if (!created) {
        auto err = ErrorContext::warning(ErrorCode::DatabaseQueryFailed,
                                         "Failed to create FTS index (search will use LIKE)",
                                         origin)
                       .withDetails(q.lastError().text());
        ErrorUtils::logError(err);
      } else {
        ...
        // Mark FTS as not-yet-ready and backfill incrementally on the scan worker.
        // Avoid blocking app startup by running a full synchronous rebuild here.
        ensureIndex(db,
                    "INSERT OR IGNORE INTO meta(key, value) VALUES('items_fts_ready', '0')",
                    origin, "meta items_fts_ready");
        ensureIndex(db,
                    "INSERT OR IGNORE INTO meta(key, value) VALUES('items_fts_indexed_up_to_id', '0')",
        ...
        ensureIndex(db,
                    ...
                    "  INSERT INTO items_fts(items_fts, rowid, name, path, collection_uuid) "
                    "  VALUES('delete', old.id, old.name, old.path, old.collection_uuid); "
                    "END;",
                    origin, "items_fts_ad");
