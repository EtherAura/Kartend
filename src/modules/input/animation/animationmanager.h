...
//
// QObject must be the first base; IAnimationManager is a plain (non-QObject)
...
public:
  ...
  void ensureVAnimCreated(QScrollBar *vScrollBar) override;
  ...
  void configureAndStartVerticalAnimation(QScrollBar *vScrollBar, int curY, int targetY,
                                          ...
                                          bool clickHoldAdv) override;

  /// Stops active vertical animations on the scrollbar
  void stopActiveVerticalAnims(QScrollBar *verticalScrollBar) override;

  /// Returns true if vertical animation is currently running
  [[nodiscard]] bool isVerticalAnimRunning() const override;
  ...
  [[nodiscard]] bool handleExistingVerticalAnimIfRunning(QScrollBar *verticalScrollBar, int targetY,
                                                         ...
                                                         int &curY, int &distance) override;

  /// Returns the current vertical animation (may be nullptr)
  [[nodiscard]] QPropertyAnimation *verticalAnimation() const override { return m_vScrollAnim; }
  ...
  void initHorizontalAnimIfNeeded(QScrollBar *hScrollBar) override;

  /// Animates horizontal scrolling for hold mode
  void animateHorizontalHold(QScrollBar *hScrollBar, int startX, int targetX) override;

  /// Animates horizontal scrolling with smooth easing
  void animateHorizontalSmooth(QScrollBar *hScrollBar, int startX, int targetX) override;

  /// Returns true if horizontal animation is currently running
  [[nodiscard]] bool isHorizontalAnimRunning() const override;

  /// Returns the current horizontal animation (may be nullptr)
  [[nodiscard]] QPropertyAnimation *horizontalAnimation() const override { return m_hScrollAnim; }
  ...
  void startEnsureVisibleVAnim(QScrollBar *vScrollBar, int startVal, int endVal, int itemHeight,
                               int verticalSpacing, bool isRepeating) override;
