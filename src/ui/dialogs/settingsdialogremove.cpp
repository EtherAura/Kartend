...
auto SettingsDialog::restoreExpandedStates(const QList<int> &expandedBefore, int removedIndex)
    -> void {
  for (int expIdx : expandedBefore) {
    ...
    if (collectionIndexToItem.contains(adjustedIdx) && (collectionIndexToItem[adjustedIdx])) {
...
auto SettingsDialog::selectTargetAfterRemoval(int parentIdx, int removedIndex) -> void {
  if (parentIdx >= removedIndex) {
    parentIdx -= 1;
  }
  int targetIndex = (parentIdx >= 0 && parentIdx < collections.size()) ? parentIdx : 0;
  currentCollectionIndex =
      (collections.isEmpty()) ? -1 : qBound(0, targetIndex, collections.size() - 1);

  if (currentCollectionIndex >= 0 && collectionIndexToItem.contains(currentCollectionIndex)) {
...
void SettingsDialog::removeCollection() {
  ...
  QMessageBox::StandardButton reply =
      QMessageBox::question(this, "Remove Collection", message, QMessageBox::Yes | QMessageBox::No);
