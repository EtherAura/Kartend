...
| `autoScrapeEntityArtOnCreate` | bool | `true` | Fetch a collection's logo and background in the background when the collection is created (settings dialog, duplicate, `.kart` import, launcher import). Silent — no dialog, no completion box. Only collections that resolve a scraper and have no artwork set yet are fetched, so a hand-picked icon is never replaced and a rename never re-fetches. Off = artwork is fetched only when you ask for it. |
