...
[subfolder artwork generator](../dev/subfolder-artwork.md).
...
- The C++ struct is `CollectionConfig` in
  [src/utils/app/collection/collectionconfig.h](../../src/utils/app/collection/collectionconfig.h).
  Every key in this page maps 1:1 to a struct member.
- Hierarchy traversal goes through `CollectionHierarchyCache`
  (`collection/collectionhierarchycache.h`) and `NavigationStackManager`
...
- Virtual subfolder collections are synthesized at scan time by
  ...
  in `collection/collectionconfig.h` to follow the runtime-only field that
  drives them.
